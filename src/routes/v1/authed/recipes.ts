  ...
  fastify.get(
    ...
    async (request: FastifyRequest<{ Querystring: IRecipeFilter }>, reply) => {
      ...
      if (minCalories !== undefined)
        exprConditions.push({ $gte: [nutrientVal("calories"), minCalories] });
      if (maxCalories !== undefined)
        exprConditions.push({ $lte: [nutrientVal("calories"), maxCalories] });
      if (minProtein !== undefined)
        exprConditions.push({
          $gte: [nutrientVal("proteinContent"), minProtein],
        });
      if (maxProtein !== undefined)
        exprConditions.push({
          $lte: [nutrientVal("proteinContent"), maxProtein],
        });
      if (minCarbs !== undefined)
        exprConditions.push({
          $gte: [nutrientVal("carbohydrateContent"), minCarbs],
        });
      if (maxCarbs !== undefined)
        exprConditions.push({
          $lte: [nutrientVal("carbohydrateContent"), maxCarbs],
        });
      if (minFat !== undefined)
        exprConditions.push({ $gte: [nutrientVal("fatContent"), minFat] });
      if (maxFat !== undefined)
        exprConditions.push({ $lte: [nutrientVal("fatContent"), maxFat] });
      if (exprConditions.length === 1) {
        matchObj.$expr = exprConditions[0];
      } else if (exprConditions.length > 1) {
        matchObj.$expr = { $and: exprConditions };
      }
      ...
      if (sort) {
        switch (sort) {
          ...
          case "-calories": {
            ...
            sortObj = { _sortVal: sort.startsWith("-") ? -1 : 1 };
            break;
          }
          case "protein":
          case "-protein": {
